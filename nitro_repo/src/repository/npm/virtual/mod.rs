...
impl<C: VirtualMemberClient> VirtualResolver<C> {
    ...
    pub async fn resolve(
        ...
        path: &StoragePath,
        method: Method,
    ) -> Result<Option<ResolveHit>, NPMRegistryError> {
...
fn is_auth_failure(response: &RepoResponse) -> bool {
    matches!(response, RepoResponse::Other(resp) if resp.status() == StatusCode::UNAUTHORIZED || resp.status() == StatusCode::FORBIDDEN)
}
...
async fn validate_publish_target(
    ...
    if !members.iter().any(|m| m.repository_id == target) {
        return Err(RepositoryFactoryError::InvalidConfig(
            NPMRegistryConfigType::get_type_static(),
