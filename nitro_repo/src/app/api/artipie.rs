...
pub fn routes() -> Router<NitroRepo> {
    Router::new()
...
#[cfg(test)]
mod tests {
    use super::{build_repository_location, derive_metadata_path};

    #[test]
    fn build_location_with_tail_and_query() {
        let location = build_repository_location(
            "storage",
            "repo",
            "group/artifact/file.jar",
            Some("param=value"),
        );
        assert_eq!(
            location,
            "/repositories/storage/repo/group/artifact/file.jar?param=value"
        );
    }

    #[test]
    fn build_location_without_tail() {
        let location = build_repository_location("storage", "repo", "", None);
        assert_eq!(location, "/repositories/storage/repo");
    }

    #[test]
    fn derive_metadata_with_nested_group() {
        let path = derive_metadata_path("org/apache/maven/plugins/maven-resources-plugin")
    ...
    #[test]
    fn derive_metadata_with_root_group() {
        let path = derive_metadata_path("artifact").expect("metadata path");
        assert_eq!(path, "artifact/maven-metadata.xml");
    }

    #[test]
    fn derive_metadata_rejects_invalid_values() {
        assert!(derive_metadata_path("").is_none());
        assert!(derive_metadata_path("group/").is_none());
        assert!(derive_metadata_path("/group/").is_none());
    }
}
