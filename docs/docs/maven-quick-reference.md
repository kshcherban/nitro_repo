  ...
  <profiles>
    <profile>
      ...
      <repositories>
        <repository>
          <id>nitro-repo</id>
          <url>https://your-nitro-repo.example.com/repositories/storage/maven-repo</url>
        </repository>
      </repositories>
    </profile>
  </profiles>
  <activeProfiles>
    <activeProfile>nitro-repo</activeProfile>
  </activeProfiles>
</settings>
```
...
mvn deploy:deploy-file \
  -Durl=https://your-nitro-repo.example.com/repositories/storage/maven-repo \
  -DrepositoryId=nitro-repo \
...
mvn dependency:get \
  ...
  -DremoteRepositories=https://your-nitro-repo.example.com/repositories/storage/maven-repo
```

### Test Repository Access
```bash
# Test connectivity
curl -I "https://your-nitro-repo.example.com/repositories/storage/maven-repo"

# Download artifact metadata
curl -u "username:password" \
     "https://your-nitro-repo.example.com/repositories/storage/maven-repo/com/example/myapp/maven-metadata.xml"

# Download specific JAR
curl -u "username:password" -O \
     "https://your-nitro-repo.example.com/repositories/storage/maven-repo/com/example/myapp/1.0.0/myapp-1.0.0.jar"
```
  ...
  build-and-deploy:
    ...
    steps:
      ...
      - name: Deploy to Maven Repository
        run: mvn deploy
        env:
          MAVEN_USERNAME: ${{ secrets.NITRO_USERNAME }}
          MAVEN_PASSWORD: ${{ secrets.NITRO_PASSWORD }}
```
...
# Check root metadata
curl -u "username:password" \
     "https://your-nitro-repo.example.com/repositories/storage/maven-repo/maven-metadata.xml"
...
curl -u "username:password" -I \
     "https://your-nitro-repo.example.com/repositories/storage/maven-repo/"

# Test upload permissions
echo "test" | curl -u "username:password" -T - \
     "https://your-nitro-repo.example.com/repositories/storage/maven-repo/test/test.txt"
```
...
mvn deploy \
  -DaltDeploymentRepository=nitro-repo::default::https://your-nitro-repo.example.com/repositories/storage/maven-repo
```
...
| Require Nitro Deploy | false | true for automated deployments |
