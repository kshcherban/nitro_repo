...
| Docker / OCI | Yes | Planned | Private Docker Registry HTTP API v2 implementation; proxy mode is on the roadmap ([docs](./repositoryTypes/docker/index.md)). |
...
| --- | --- | --- |
| Project Pages | Yes | Repositories surface uploaded components in the UI, enabling per-project landing pages with metadata and badges. |
...
| Badge Generation | Yes | Configurable styles provided via `/api/badge` to advertise repository status in README badges. |
