...
3. **Archive integrity**:
   ...
   - Nitro rebuilds the archive with the canonical structure `<module>@<version>/…` and inserts the resolved `go.mod`.
4. **Metadata synthesis**: if `info` is absent Nitro generates:
   ```json
   {
     "Version": "v1.2.3",
...
Nitro also maintains `@v/list`, a newline-delimited list of published versions for the module. This file is regenerated whenever a new version is published.
...
3. `GET /@v/v1.2.3.mod`
4. `GET /@v/v1.2.3.zip`
5. Optional sumdb lookups if `GOSUMDB` is enabled (Nitro currently returns `501` for hosted sumdb).
