...
Nitro Repo validates signed JWT/ID tokens from upstream SSO providers (Cloudflare Access, Okta, Auth0, Azure B2C, etc.) using JWKS. Tokens are verified per-provider, mapped to a Nitro user, optional Casbin roles are applied, and a Nitro Repo session cookie is issued.

## Configuration

Configure the `security.sso` block inside `cfg/nitro_repo.toml` (or via Admin → System → Single Sign-On):
...
Header-based SSO still works, but Nitro Repo can now validate signed ID tokens directly against a provider's JWKS endpoint. Add one or more providers under `security.sso.providers`:
...
- `jwks_url`: JWKS endpoint. If omitted Nitro Repo will try `/.well-known/openid-configuration` discovery on the issuer.
...
Keys fetched from JWKS are cached for one hour and refreshed automatically when they expire or a new `kid` shows up. If no provider yields a valid token, Nitro Repo falls back to the legacy header-based extraction above.
...
- Every authenticated request also carries a signed `Cf-Access-Jwt-Assertion` header that Cloudflare validates before forwarding to your origin. Nitro Repo now re-validates that signature against Cloudflare’s JWKS so the token cannot be spoofed.

Configure Nitro Repo to validate the Cloudflare Access token via JWKS:
...
Cloudflare automatically blocks unauthenticated traffic, so Nitro Repo only receives requests that satisfy your Access policies. The JWKS-backed provider above re-validates the `Cf-Access-Jwt-Assertion` signature; no trust in plain headers is required.
