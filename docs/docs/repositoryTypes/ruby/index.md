...
Nitro Repo can host a RubyGems-compatible repository for private gems or act as a proxy cache in
...
- `gem push` / `gem yank` are supported via the RubyGems API endpoints under the repository base
  path.
- Uploaded `.gem` files are stored under `gems/` and indexed into the Nitro package catalog
...
- GET/HEAD requests for supported index files (Compact Index + legacy RubyGems index) and `.gem`
  downloads are served from cache if present; otherwise Nitro fetches from upstream and caches the
  response.
...
```bash
bundle config set --global https://<host>/repositories/<storage>/<repo>/ token:<nitro_token>
```

Nitro accepts HTTP Basic auth for repository routes; the password may be a Nitro auth token.
