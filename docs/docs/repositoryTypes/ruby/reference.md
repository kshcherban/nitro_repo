...
### Bundler: proxy rubygems.org through Nitro (proxy repo)
```bash
...
curl --data-binary @mygem-1.2.3.gem \
  -H "Authorization: <nitro_token>" \
  https://<host>/repositories/<storage>/<repo>/api/v1/gems
```

### Publish (gem CLI)
Configure a Nitro auth token as your RubyGems API key for this host (for example via
