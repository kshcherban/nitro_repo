...
Virtual NPM repositories let you present several hosted or proxy NPM repositories as a single endpoint. Clients point to the virtual repo URL and Nitro Repo resolves packages across the configured members.
...
always-auth=true
```
Publishing uses the same URL; Nitro forwards to the configured hosted member.
