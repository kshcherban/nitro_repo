...
Nitro Repo can run as an NPM-compatible registry for private packages or act as a lightweight cache
...
- Upload tarballs by pointing your `.npmrc` to
  `/repositories/<storage>/<repository>/` (Nitro Repo expects the trailing slash).
...
- Configure one or more upstream URLs (for example `https://registry.npmjs.org`) in the repository
  settings.
- GET/HEAD requests are served from Nitro Repo if the asset is cached; otherwise the request is
  proxied upstream and cached locally for subsequent access.
- Nitro Repo enforces its own permission checks before reaching the upstream registry.
