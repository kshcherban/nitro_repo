...
- Proxy repos are read-only. Dist files and p2 metadata are cached on first request and served from Nitro with rewritten `dist.url` values.
...
- First metadata request (`p2/<vendor>/<package>.json`) is fetched from upstream, rewritten so `dist.url` points at Nitro, stored under `p2/...`, and indexed in the catalog.
- First dist download streams from upstream while being cached under `dist/<vendor>/<package>/<version>/<filename>`.
- Subsequent installs are served from Nitro’s cache; upstream is not hit again unless the cache entry is removed.
