# Nitro Repo Integration Test Suite

Comprehensive end-to-end integration tests for Nitro Repo, testing all supported package types using native package management tooling.

## Overview

This test suite validates the complete functionality of Nitro Repo by testing real-world package management workflows using native tools (maven, npm, docker, helm, go, pip, composer) in isolated Docker containers.
...
│   ├── config/
│   │   └── nitro_repo.test.toml    # Test server configuration
...
2. **nitro_repo** - Nitro Repo server built from local source
...
All containers communicate on the `test-network` bridge network. Tests run inside the `test-runner` container with access to `nitro_repo:8888`.
...
**Utilities:**
- `wait_for_server([max_wait])` - Wait for Nitro Repo to be healthy
...
# Check Nitro Repo logs
docker compose -f tests/docker/docker-compose.test.yml logs nitro_repo
...
4. Check Nitro Repo: `curl http://nitro_repo:8888/api/health`
5. Check database: `docker compose exec postgres psql -U nitro -d nitro_repo_test`
...
Part of Nitro Repo project. See main project LICENSE file.
