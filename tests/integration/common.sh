...
# Test configuration
export NITRO_URL="${NITRO_URL:-http://nitro_repo:8888}"
...
wait_for_server() {
    ...
    while [ $elapsed -lt $max_wait ]; do
        if curl -sf "${NITRO_URL}/" > /dev/null 2>&1; then
...
# Arguments:
#   $1: URL path (relative to NITRO_URL)
...
api_get() {
    local path="$1"
    shift
    curl -sf -H "$(get_auth_header)" "${NITRO_URL}${path}" "$@"
}
...
api_put() {
    local path="$1"
    shift
    curl -sf -X PUT -H "$(get_auth_header)" "${NITRO_URL}${path}" "$@"
}
...
api_post() {
    local path="$1"
    shift
    curl -sf -X POST -H "$(get_auth_header)" "${NITRO_URL}${path}" "$@"
}
...
api_delete() {
    local path="$1"
    shift
    curl -sf -X DELETE -H "$(get_auth_header)" "${NITRO_URL}${path}" "$@"
}
...
create_workspace() {
    local base_name="$1"
    local workspace="/tmp/nitro-test-${base_name}-$$"
    mkdir -p "$workspace"
    echo "$workspace"
}
