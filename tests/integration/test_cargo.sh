...
cd "${CRATE_DIR}"
if CARGO_REGISTRIES_NITRO_TOKEN="${TEST_TOKEN}" run_cmd cargo publish --registry nitro --allow-dirty; then
    pass
else
...
INDEX_CONTENT=$(curl -sf "${NITRO_URL}/repositories/${CARGO_REPO}/${INDEX_PATH}" || true)
...
CRATE_METADATA=$(curl -sf "${NITRO_URL}/repositories/${CARGO_REPO}/api/v1/crates/${CRATE_NAME}" || true)
...
if curl -sfL "${NITRO_URL}/repositories/${CARGO_REPO}/api/v1/crates/${CRATE_NAME}/${VERSION}/download" -o "${DOWNLOAD_PATH}" && \
...
if CARGO_REGISTRIES_NITRO_TOKEN="${TEST_TOKEN}" run_cmd cargo install --registry nitro "${CRATE_NAME}" --version "${VERSION}" --force; then
    pass
else
...
if [[ -x "${INSTALLED_BIN}" ]] && OUTPUT=$("${INSTALLED_BIN}"); then
    ...
    if grep -q "nitro-cargo-test" <<<"${OUTPUT}"; then
        clear_last_log
        pass
    else
