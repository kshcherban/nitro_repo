...
Priority: optional
Architecture: ${ARCH}
Maintainer: Nitro Repo <test@nitro.test>
Description: Minimal package for Nitro Repo integration tests
CONTROL
    cat >"${PKG_BUILD_DIR}/usr/local/bin/${PKG_NAME}" <<'SCRIPT'
#!/bin/bash
echo "Nitro Repo Debian integration test ${VERSION}"
SCRIPT
...
UPLOAD_STATUS=$(get_http_status "${NITRO_URL}/repositories/${DEB_REPO}" \
    -X POST \
    -H "$(get_auth_header)" \
...
PACKAGES_URL="${NITRO_URL}/repositories/${DEB_REPO}/dists/stable/main/binary-${ARCH}/Packages"
...
RELEASE_CONTENT=$(curl -sf "${NITRO_URL}/repositories/${DEB_REPO}/dists/stable/Release" || true)
...
POOL_URL="${NITRO_URL}/repositories/${DEB_REPO}/pool/main/${FIRST_LETTER}/${PKG_NAME}/${DEB_FILE}"
