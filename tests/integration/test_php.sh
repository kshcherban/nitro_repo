...
evict_php_proxy_metadata_cache() {
    ...
    if ! response=$(
        curl -fsS -X DELETE \
            ...
            "${NITRO_URL}/api/repository/${proxy_repo_id}/packages" \
            --data "$payload" 2>&1
    ); then
...
fetch_dist_url_from_metadata() {
    ...
    local meta
    if ! meta=$(curl -fsS "${NITRO_URL}${meta_path}" 2>&1); then
        record_output "$meta"
        return 1
    fi
    local dist_url
...
fetch_proxy_upstream_url_from_metadata() {
    ...
    if ! upstream_url=$(
        jq -r --arg name "$PACKAGE_NAME" --arg ver "$version" \
            '([.packages[$name][]? | select(.version == $ver) | .dist["nitro-upstream-url"]] | last) // empty' <<<"$meta"
    ); then
        record_output "$meta"
        return 1
    fi
...
cat > "$CONSUMER_PROXY_DIR/composer.json" <<EOF
{
  ...
  "repositories": [
    { "type": "composer", "url": "${NITRO_URL}/repositories/${PHP_PROXY_REPO}" }
  ],
  "require": { "${PACKAGE_NAME}": "${VERSION_1}" },
  "config": { "secure-http": false }
}
...
if ! UPSTREAM_URL=$(fetch_proxy_upstream_url_from_metadata "${VERSION_1}"); then
    fail "Failed to read nitro-upstream-url from proxy metadata for ${VERSION_1}"
    popd >/dev/null
    finish
    exit 1
fi
if [[ -n "$UPSTREAM_URL" && "$UPSTREAM_URL" =~ ^https?:// && "$UPSTREAM_URL" == *"/repositories/${PHP_HOSTED_REPO}/"* ]]; then
    ...
    fail "Expected nitro-upstream-url to point at hosted repo, got: ${UPSTREAM_URL:-<empty>}"
    popd >/dev/null
    finish
    exit 1
fi
...
if OUTPUT_PROXY=$(php -r "require 'vendor/autoload.php'; echo NitroTest\\SampleLib\\Greeter::getVersion();" 2>&1); then
...
if OUTPUT_PROXY_V2=$(php -r "require 'vendor/autoload.php'; echo NitroTest\\SampleLib\\Greeter::getVersion();" 2>&1); then
