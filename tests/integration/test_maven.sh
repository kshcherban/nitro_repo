...
sed -i "/<\/project>/i \
  <distributionManagement>\
    <repository>\
      <id>nitro-maven-hosted</id>\
      <url>${NITRO_URL}/repositories/${MAVEN_HOSTED_REPO}</url>\
    </repository>\
...
DOWNLOAD_PATH="${NITRO_URL}${UPLOAD_PATH}"
...
if curl -sf "${NITRO_URL}${UPLOAD_PATH}" > /dev/null && \
   curl -sf "${NITRO_URL}${UPLOAD_PATH_V2}" > /dev/null; then
    pass
else
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <groupId>com.nitro.test</groupId>
...
if curl -sf "${NITRO_URL}${PROXY_ARTIFACT_PATH}" -o "$WORKSPACE/proxied.jar" && \
...
if curl -sf "${NITRO_URL}${PROXY_ARTIFACT_PATH}" -o "$WORKSPACE/proxied2.jar" && \
