...
RUN_ID="$(date +%s)"
PACKAGE_NAME="nitro-virtual-test-pkg-${RUN_ID}"
...
sed -i "s/name='nitro-test-pkg'/name='${PACKAGE_NAME}'/" setup.py
sed -i "s/version='1.0.0'/version='${VERSION_1}'/" setup.py
sed -i "s/__version__ = '1.0.0'/__version__ = '${VERSION_1}'/" nitro_test_pkg/__init__.py
...
[nitro-hosted-2]
repository: ${NITRO_URL}/repositories/${PYTHON_HOSTED_2_REPO}
username: ${TEST_USER}
password: ${TEST_PASSWORD}

[nitro-virtual]
repository: ${NITRO_URL}/repositories/${PYTHON_VIRTUAL_REPO}
username: ${TEST_USER}
password: ${TEST_PASSWORD}
EOF

if run_cmd twine upload --repository nitro-hosted dist/*; then
    pass
else
...
if ! run_cmd python3 setup.py sdist bdist_wheel; then
    ...
    if run_cmd twine upload --repository nitro-hosted-2 dist/*; then
        pass
    else
...
VIRTUAL_INDEX=$(curl -sf "${NITRO_URL}${SIMPLE_VIRTUAL_PATH}" || echo "")
...
if run_cmd pip install --index-url="${NITRO_URL}/repositories/${PYTHON_VIRTUAL_REPO}/simple" \
...
sed -i "s/__version__ = '${VERSION_2}'/__version__ = '${VERSION_3}'/" nitro_test_pkg/__init__.py
rm -rf dist/ build/ *.egg-info

if ! run_cmd python3 setup.py sdist bdist_wheel; then
    fail "Failed to rebuild publish-forwarding artifacts"
else
    if run_cmd twine upload --repository nitro-virtual dist/*; then
        pass
    else
...
HOSTED_INDEX=$(curl -sf "${NITRO_URL}${SIMPLE_HOSTED_PATH}" || echo "")
