...
CREATE_STATUS=$(curl -s -o "$WORKSPACE/new-repo.json" -w "%{http_code}" \
    ...
    "${NITRO_URL}/api/repository/new/docker" \
    -d "$NEW_REPO_JSON")
...
if [[ "$CREATE_STATUS" == "201" ]]; then
    ...
    fail "Failed to create auth-enabled docker proxy repository (status=$CREATE_STATUS). This may indicate an incompatible or older Nitro Repo image; see recorded output for details."
fi
...
if [[ -n "$NEW_PROXY_IMAGE" ]]; then
    ...
    if run_cmd docker pull "${NEW_PROXY_IMAGE}"; then
        ...
        fail "Failed to pull image through auth-enabled proxy repository. If the Nitro Repo container crashed or restarted, you may be testing against an older image without the Docker proxy auth fix."
    fi
else
...
if [[ -n "$NEW_REPO_PATH" ]]; then
    V2_MANIFEST_URL="${NITRO_URL}/v2/${NEW_REPO_PATH}/${UPSTREAM_IMAGE}/manifests/latest"
    ...
    if run_cmd curl -sf -H "$(get_auth_header)" "$V2_MANIFEST_URL" >/dev/null; then
        ...
        fail "Failed to GET manifest via auth-enabled proxy using Bearer token; this likely indicates an older Nitro Repo image or a regression in Docker proxy manifest handling."
    fi
else
