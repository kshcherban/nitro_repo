...
MODULE_NAME="nitro.test/test-module"
VERSION_1="v1.0.0"
VERSION_2="v1.0.1"
...
STATUS=$(get_http_status "${NITRO_URL}${INFO_PATH}" \
    -X PUT \
    -H "$(get_auth_header)" \
...
STATUS=$(get_http_status "${NITRO_URL}${MOD_PATH}" \
    -X PUT \
    -H "$(get_auth_header)" \
...
export GOPROXY="${NITRO_URL}/repositories/${GO_HOSTED_REPO}"
export GOSUMDB="off"
...
export GOPROXY="${NITRO_URL}/repositories/${GO_PROXY_REPO}"
export GOSUMDB="off"
